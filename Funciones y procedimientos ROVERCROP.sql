...
DELIMITER $$
create procedure UsuarioYContraseña()
begin 
select u.identificacion, u.nombre,u.apellido,concat(upper(substr(u.nombre,1,4)),upper(substr(u.apellido, 1,2)),substr(u.identificacion, 1,3))"usuario",
concat(substr(u.identificacion, 1,4),".",upper(substr(u.apellido, 1,2)))"contraseña"
from usuario as u;
end $$
...
DELIMITER //
CREATE FUNCTION edadUsuario(fecha_nacimiento date) RETURNS int
    DETERMINISTIC
begin 
    declare edad int;
...
select mydb.edadUsuario(usuario.fecha_nacimiento) as edad, usuario.identificacion,usuario.nombre from usuario
